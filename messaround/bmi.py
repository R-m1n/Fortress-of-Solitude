...
def button_callback():
    weight = 0 if entry_1.get() == '' else float(entry_1.get())
    height = 1 if entry_2.get() == '' else float(entry_2.get())

    wmetric = combobox_1.get()
    if wmetric == "g":
        weight /= 1000
...
def create_toplevel(bmi):
    ...
    label = customtkinter.CTkLabel(window, text=f"BMI:\t{bmi}")
